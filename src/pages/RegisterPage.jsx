...
function RegisterPage() {
  ...
  const { loading, error, success, message } = useSelector( (state) => state.auth.register);
  ...
  const validateField = (fieldName) => {
    const value = fieldName === 'name' ? name : fieldName === 'email' ? email : 
      fieldName === 'password' ? password : fieldName === 'phone' ? phone : region;
    ...
    if (fieldName === 'password' && (!value.trim() || value.trim().length < 8 || 
